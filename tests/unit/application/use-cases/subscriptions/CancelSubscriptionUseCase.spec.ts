...
const setup = () => {
  ...
  const useCase = new CancelSubscriptionUseCase(subscriptionRepo, eventPublisher);

  return { useCase, subscriptionRepo, eventPublisher };
};

describe('CancelSubscriptionUseCase', () => {
  it('cancels subscription and publishes event', async () => {
    ...
    expect(response.status).toBe(SubscriptionStatusValue.CANCELLED);
    expect(response.endDate.getTime()).toBeGreaterThanOrEqual(effectiveDate.getTime());
  });
